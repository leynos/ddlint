...
| Legacy token policy (`typedef`, `as`, legacy type names, `#`, `<=>`) | Spec marks several as rejected/compatibility managed (`docs/differential-datalog-parser-syntax-spec-updated.md:446`)                                      | Tokenizer still recognises them and parser behaviour is mixed (`src/tokenizer.rs:119`)                                                                    | Must be formalised as compatibility policy pre-split                         |
