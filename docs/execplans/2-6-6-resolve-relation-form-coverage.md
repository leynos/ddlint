...
| D2  | `PrimaryKey ::= '[' 'primary' 'key' '(' LcName ')' Expr ']'` | `primary key (var_name) expr` — no surrounding brackets, single lambda binder, restricted to `input` relations | Update spec: drop the outer brackets; the local contract accepts a comma-separated list of one or more `LcName` binders (extending upstream's single lambda-style binder) followed by an optional expression; record the input-only static check. |
